...
public class LocaleUtils {
  ...
  // any length
  public final static Pattern LOCALE_PATTERN = Pattern.compile("(\\w{2})(_(\\w{2}))?(_(\\w+))?"); //$NON-NLS-1$
  ...
  public LocaleUtils(final Locale locale, final Boolean devMode, final String bundleRoot) {
    setLocale(locale);
    setDevMode(devMode);
  ...
  public LocaleUtils(final Locale locale, final Boolean devMode, final ResourceBundle bundle) {
    setLocale(locale);
    setDevMode(devMode);
    this.bundle = bundle;
  }
  ...
  /**
   * This retrieves a message from the appropriate properties file for your given Locale. It also
   * uses any provided arguments to make substitutions and formatting changes to the found message.
   * Note that this uses the OGNL expression language syntax for formatting.
   * 
   * @param key - properties key used to look up message
   * @param arguments - an array of arguments to be used in the message, uses OGNL syntax
  ...
  private String getText(final String key, final ResourceBundle bundle, Object... arguments) {
   ...
   * 
   * @param memory The memory value to format.
   * @param override The locale to be used (determines decimal and grouping separators).
