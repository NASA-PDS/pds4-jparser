...
public class StrUtils {
	...
	public static Number getNumber(final String string, final Locale locale) {
		try {
			...
			return NumberFormat.getInstance(locale).parse(
					string.replaceAll("\\+", ""));
	...
	public static Number getNumberLoose(final String string) {
		...
		try {
			return NumberFormat.getInstance(new Locale("en_us")).parse(
					numberVal);
		} catch (ParseException e) {
			return null;
		}
	...
	public static List<String> toStatements(final String sql) {
		...
		while (matcher.find()) {
			final String currentMatch = sql.substring(matcher.start(), matcher
					.end());
