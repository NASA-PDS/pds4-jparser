...
public class FileUtils {
  ...
  public static boolean containsFile(final File sourceFile, final String searchName) {
  ...
  public static List<File> getFiles(final File sourceFile, final String searchPattern) {
    return getFiles(sourceFile, searchPattern, true);
  }

  public static List<File> getFiles(final File sourceFile, final String regex,
  ...
  public static List<File> getFilesByPattern(final File sourceFile, final Pattern searchPattern,
      final boolean recursive) {
    if (!sourceFile.exists()) {
      throw new RuntimeException("File \"" + sourceFile.toString() + "\" was not found.");
    }
    final Stack<File> dirStack = new Stack<>();
  ...
  public static File getTopFileByPattern(final File sourceFile, final String regex,
      final Integer maxDepth) {
    final Pattern searchPattern = Pattern.compile(regex, Pattern.CASE_INSENSITIVE);
    return getTopFileByPattern(sourceFile, searchPattern, maxDepth);
  }

  // return first file that looks right up to a given depth
  public static File getTopFileByPattern(final File sourceFile, final Pattern searchPattern,
      final Integer maxDepth) {
    final Stack<File> dirStack = new Stack<>();
    dirStack.push(sourceFile);
  ...
  public static List<File> getFiles(final List<File> sourceList, final String regex) {
  ...
  public static Map<Integer, File> getFileMap(final File sourceFile, final String searchPattern) {
    return getFileMap(sourceFile, searchPattern, true);
  }

  public static Map<Integer, File> getFileMap(final File sourceFile, final String regex,
      final boolean recursive) {
    Pattern searchPattern = null;
    if (regex != null) {
      searchPattern = Pattern.compile(regex, Pattern.CASE_INSENSITIVE);
  ...
  public static Map<Integer, File> getFileMapByPattern(final File sourceFile,
    ...
    final Map<Integer, File> foundFiles = new HashMap<>();

    final boolean doSearch = searchPattern != null;

    // check root folder - has to be recursive for sourcefile to add itself
    // as folder to search below
    conditionalAddFile(sourceFile, dirStack, foundFiles, searchPattern, doSearch, true,
    ...
    while (!dirStack.empty()) {
      ...
      final File[] tempFiles = curDir.listFiles();
      for (final File curFile : tempFiles) {
        conditionalAddFile(curFile, dirStack, foundFiles, searchPattern, doSearch, recursive,
            excludeSourceControlFiles);
      }
    }
    return foundFiles;
  ...
  private static void conditionalAddFile(final File curFile, final Stack<File> dirStack,
      final Map<Integer, File> foundFiles, final Pattern searchPattern, final boolean doSearch,
    ...
    if (doSearch) {
      final boolean matches = searchPattern.matcher(curFile.getName()).matches();
      if (matches) {
        if (!excludeSourceControlFiles || !isSourceControl(curFile)) {
          foundFiles.put(curFile.hashCode(), curFile);
        }
      }
    } else if (!excludeSourceControlFiles || !isSourceControl(curFile)) {
      foundFiles.put(curFile.hashCode(), curFile);
    }
  }

  // get files from list that match regex
  public static Map<Integer, File> getFileMap(final Map<Integer, File> sourceList,
      final String regex) {
    final Map<Integer, File> matchingFiles = new HashMap<>();
    final Pattern searchPattern = Pattern.compile(regex, Pattern.CASE_INSENSITIVE);
  ...
  public static String getRelativePath(final File baseDirectory, final File targetFile) {
  ...
  public static String getRelativePath(final URL baseDirectory, final URL targetFile) {
    if (baseDirectory.equals(targetFile)) {
      return ""; //$NON-NLS-1$
    }
  ...
  public static String getRelativePath(final String basePath, final String fullPath) {
    if (basePath.equals(fullPath)) {
      return ""; //$NON-NLS-1$
    }
    final String normalBasePath = basePath.replaceAll(OPPOSITE_SEP_CHAR, REGEX_SAFE_SEP);
    final String normalFullPath = fullPath.replaceAll(OPPOSITE_SEP_CHAR, REGEX_SAFE_SEP);
    if (normalFullPath.indexOf(normalBasePath) == -1) {
      // try normalizing paths

      throw new RuntimeException("target file '" + normalFullPath
          + "' is not a child of provided base directory '" + normalBasePath + "'.");
    }
  ...
  public static boolean isParent(final FileMirror parent, final FileMirror searchFile) {
    if (parent.equals(searchFile)) {
      return true;
    }
    if (!parent.isDirectory()) {
  ...
  public static boolean hasParent(final List<File> parents, final File searchFile) {
  ...
  public static String fromVaxPath(final String vaxPath) {
    ...
    String returnString =
        vaxPath.replaceAll("(\\[.*)(\\.)(.*\\].*)", "$1" + getRegexSeparator() + "$3");
    // replace substitution markers
    returnString = returnString.replaceAll("\\[(.*)\\](.*)", "$1" + getRegexSeparator() + "$2");
    return returnString;
  }
  ...
  public static File getCaseUnknownFile(final File rootFile, final String searchName) {
  ...
  public static File getAlternateCaseFile(final File rootFile, final String searchName) {
  ...
  public static boolean empty(final File directory) {
    ...
    if ((files.length == 0) || (files.length == 1 && files[0].getName().equals(".svn"))) { //$NON-NLS-1$
      return true;
    }
    return false;
  }
